...
import {ITableBatch, PropsMainPageBillAll} from './interfaces';
...
function MainPageBillAll({}: PropsMainPageBillAll) {
	...
	const getUrlUpdateBatchBill = (data: ITableBatch): string => {
		if (data.scalesType == TYPE_SCALES.CAN_NHAP) {
			return `/phieu-du-kien/chinh-sua-phieu-nhap?_id=${data.uuid}`;
		}
		if (data.scalesType == TYPE_SCALES.CAN_XUAT) {
			return `/phieu-du-kien/chinh-sua-phieu-xuat?_id=${data.uuid}`;
		}
		if (data.scalesType == TYPE_SCALES.CAN_DICH_VU) {
			return `/phieu-du-kien/chinh-sua-phieu-dich-vu?_id=${data.uuid}`;
		}
		if (data.scalesType == TYPE_SCALES.CAN_CHUYEN_KHO) {
			return `/phieu-du-kien/chinh-sua-phieu-chuyen-kho?_id=${data.uuid}`;
		}
		...
		<div className={styles.container}>
				...
				<TippyHeadless
					...
					render={(attrs: any) => (
						<div className={styles.main_option}>
							<Link onClick={() => setOpenCreate(false)} href={'/phieu-du-kien/them-phieu-nhap'} className={styles.item}>
								<div className={styles.icon}>
									<AddSquare size={20} />
								</div>
								<p>Cân nhập</p>
							</Link>
							<Link onClick={() => setOpenCreate(false)} href={'/phieu-du-kien/them-phieu-xuat'} className={styles.item}>
								<div className={styles.icon}>
									<AddSquare size={20} />
								</div>
								<p>Cân xuất</p>
							</Link>
							<Link onClick={() => setOpenCreate(false)} href={'/phieu-du-kien/them-phieu-dich-vu'} className={styles.item}>
								<div className={styles.icon}>
									<AddSquare size={20} />
								</div>
								<p>Cân dịch vụ</p>
							...
							<Link
								...
								href={'/phieu-du-kien/them-phieu-chuyen-kho'}
			...
			<div className={styles.table}>
				<DataWrapper
					...
					noti={<Noti des='Hiện tại chưa có phiếu hàng nào, thêm ngay?' disableButton />}
				>
					<Table
						data={listBatch?.data?.items || []}
						column={[
