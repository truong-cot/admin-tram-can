...
import styles from './MainUpdateCustomer.module.scss';
import {IFormUpdateCustomer, PropsMainUpdateCustomer} from './interface';

function MainUpdateCustomer({}: PropsMainUpdateCustomer) {
	const router = useRouter();

	const {_id} = router.query;

	const [form, setForm] = useState<IFormUpdateCustomer>({
		name: '',
		userUuid: '',
		director: '',
		partnerUuid: '',
	...
	useQuery([QUERY_KEY.chi_tiet_khach_hang, _id], {
		queryFn: () =>
			httpRequest({
		...
		onSuccess(data) {
			if (data) {
				...
				setProductTypes(
					data?.customerSpec
						?.filter((x: any) => x?.state == 1)
						?.map((v: any) => ({
							specUuid: v?.specUuid,
							productTypeUuid: v?.productTypeUuid,
							status: v?.status,
							state: v?.state,
						}))
				);
	...
	const handleDeleteRow = (index: number) => {
		const updatedData: any = productTypes
			?.filter((x) => x?.state == 1)
			?.map((item, i) => {
				if (i == index) {
					return {...item, state: 0};
	...
	const fucnUpdateCustomer = useMutation({
		...
		onSuccess(data) {
			if (data) {
				fucnUpdateSpecification.mutate();
			}
		},
		onError(error) {
			console.log({error});
	...
	return (
		<div className={styles.container}>
			<Loading loading={fucnUpdateCustomer.isLoading || fucnUpdateSpecification.isLoading} />
...
export default MainUpdateCustomer;
